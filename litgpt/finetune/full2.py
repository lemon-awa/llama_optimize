...
from litgpt.data import Alpaca, DataModule,FLAN
...
def main(
    ...
    fit(fabric, state, train_dataloader, val_dataloader, devices, resume, checkpoint_dir, out_dir, train, eval, data, config)
...
def fit(
    ...
    if eval.initial_validation:
        ...
        validate(fabric, model, val_dataloader, dataclasses.replace(eval, max_iters=2), tokenizer=tokenizer,verbose=False,)      # sanity check
        val_loss = "n/a"
    ...
    while state["step_count"] < max_steps and train_iterator.epoch < train.epochs:
        ...
        #     val_loss = validate(fabric, model, val_dataloader, eval, tokenizer)    
...
@torch.no_grad()
def validate(fabric: L.Fabric, model: GPT, val_dataloader: DataLoader, eval: EvalArgs, tokenizer: Tokenizer, verbose: bool = True) -> torch.Tensor:
    if verbose:
    ...
    for k, batch in enumerate(val_dataloader):
        ...
        if k == 1:
            fabric.print(f"Batch {k}:")
            for tokens in predicted_tokens:
                decoded_logits = tokenizer.decode(tokens)
                fabric.print(f"Decoded logits (prediction): {decoded_logits}")
            for target in targets:
                filtered_target = target[target != -100]
                decoded_target = tokenizer.decode(filtered_target)
                fabric.print(f"Decoded target: {decoded_target}") 
...
if __name__ == "__main__":
    config = load_config("config_hub/finetune/llama-2-7b/full.yaml")
    seed = 42
    random.seed(seed)
    np.random.seed(seed)
    ...
    setup(
        ...
        out_dir=Path(config.get("out_dir", "out/finetune/full")),
        precision=config.get("precision", "bf16-true"),
        devices=config.get("devices", 1),
        num_nodes=config.get("num_nodes", 1),
        resume=config.get("resume", False),
        data=FLAN() if config.get("data", {}).get("class_path") == "litgpt.data.FLAN" else Alpaca(),
        train=TrainArgs(**config.get("train", {})),
        eval=EvalArgs(**config.get("eval", {})),
